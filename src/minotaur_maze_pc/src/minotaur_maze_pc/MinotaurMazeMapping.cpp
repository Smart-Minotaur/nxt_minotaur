...
namespace minotaur
{
    ...
    void MinotaurMazeMapping::receivedUltrasonicData(const robot_control_beagle::UltrasonicData &p_sensorData)
    {
        if(measure)
            processSensorData(p_sensorData);
    }
    
    void MinotaurMazeMapping::processSensorData(const robot_control_beagle::UltrasonicData &p_sensorData)
    {
        RAIILock lock(&mutex);
