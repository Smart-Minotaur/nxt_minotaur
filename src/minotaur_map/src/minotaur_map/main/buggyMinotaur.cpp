...
void processMeasureSensorMsg(const minotaur_common::UltrasonicData& p_msg);
...
bool initBugzero()
{
    ...
    std::string model;
    if(!ros::param::get(minotaur::PARAM_CURRENT_MODEL(),model))
    {
    ...
    int i = 0;
    while(ros::param::has(minotaur::PARAM_SENSOR(model, i)))
    {
        float direction;
        if(!ros::param::get(minotaur::PARAM_SENSOR_DIRECTION(model,i), direction))
        {
...
bool initCommunication(ros::NodeHandle &p_handle)
{
    ...
    try {
        ...
        ROS_INFO("Subscribing on topic \"%s\"...", MINOTAUR_MEASURE_SENSOR_TOPIC);
        measureSensorSub = p_handle.subscribe(MINOTAUR_MEASURE_SENSOR_TOPIC, 100, processMeasureSensorMsg); 
...
minotaur::RobotPosition execBugAlgorithm()
{
    minotaur::RAIILock lock1(&bugMutex);
    minotaur::RAIILock lock2(&posMutex);
    return bugAlgorithm.getNextPosition(currentPosition);
}

void processMeasureSensorMsg(const minotaur_common::UltrasonicData& p_msg)
{
    minotaur::RAIILock lock(&bugMutex);
    bugAlgorithm.setSensorValue(sensorMapping[p_msg.sensorID], p_msg.distance);
}

void processOdometryMsg(const nav_msgs::Odometry& p_msgs)
{
    minotaur::RAIILock lock(&posMutex);
